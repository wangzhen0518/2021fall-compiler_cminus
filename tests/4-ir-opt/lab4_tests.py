...
def check_if_correct(exec_file, check_mode=True):
    if check_mode:
        try:
            result = subprocess.run(
                exec_file, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=10
            )
...
def table_print(testcase, before_optimization, after_optimization, baseline):
    ...
    for index, (result1, result2, result3) in enumerate(
        ...
        print(
            ...
            "\t\t%.2f" % result4 if result4 != None else "\t\tNone",
        )
...
if __name__ == "__main__":
    ...
    if usr_args.ConstPropagation:
        ...
        root_path = os.path.join(
            os.path.dirname(script_path), "testcases/ConstPropagation"
        )
        ...
        exec_files2 = compile_testcases(
            file_lists=testcases, option="-mem2reg -const-propagation"
        )
    ...
    if usr_args.LoopInvHoist:
        ...
        exec_files2 = compile_testcases(
            file_lists=testcases, option="-mem2reg -loop-inv-hoist"
        )
        results2 = evaluate(file_lists=exec_files2, metric_func=get_execute_time)
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    with open("active_vars.json", "r") as load_input:
                        with open(
                            "testcases/ActiveVars/testcase-" + str(i) + ".json", "r"
                        ) as load_answer:
                            print(
                                f"generate json {each.split('/')[-1]} \033[32;1m success\033[0m"
                            )
