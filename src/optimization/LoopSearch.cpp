...
CFGNodePtr LoopSearch::find_loop_base(CFGNodePtrSet* set,  // blocks in a loop
        ...
        if (func->get_basic_blocks().size() == 0) {
            ...
            while (strongly_connected_components(nodes, sccs)) {
                if (sccs.size() == 0) {
                    ...
                    // step 3: find loop base node for each strongly connected
                    // graph
                    for (auto scc : sccs) {
                        scc_index += 1;
                        ...
                        // step 6: remove loop base node for researching inner
                        // loop
                        reserved.insert(base);
                        dump_graph(*scc, func->get_name() + '_' +
                                             std::to_string(scc_index));
                        nodes.erase(base);
...
BBset_t* LoopSearch::get_parent_loop(BBset_t* loop) {
    ...
    for (auto prev : base->get_pre_basic_blocks()) {
        ...
        // loop is not actually a iteration, or is another unrelated iteration
