        ...
        if (instr->is_load()) {
            ...
            if (!IS_GLOBAL_VARIABLE(l_val) && !IS_GEP_INSTR(l_val)) {
                if (var_val_stack.find(l_val) != var_val_stack.end()) {
                    // why replace, it will be deleted later?
                    // to transmit the value name
                    // deleting is the last thing
...
void Mem2Reg::remove_alloca() {
    for (auto bb : func_->get_basic_blocks()) {
        ...
        for (auto instr : bb->get_instructions()) {
            ...
            if (is_alloca) {
                bool is_int = is_alloca->get_type()
                                  ->get_pointer_element_type()
                                  ->is_integer_type();
                bool is_float = is_alloca->get_type()
                                    ->get_pointer_element_type()
                                    ->is_float_type();
                if (is_int || is_float) {
                    wait_delete.push_back(instr);
